...
const PlylistModal = ({ togglePlaylistModal, video }, ref) => {	
	...
	const [isAddingToPlaylistVideo, setisAddingToPlaylistVideo] =
		useState(false);	
	...
	const addNewPlaylist = async (event) => {
		...
		if (
			dataState.userPlaylists.filter((playlist) => {
				return (
		...
		dataDispatch({
			...
			payload: { playlist: responseData.playlist },
		});
			...
			<div className="modal-bg" ref={ref}>
				<div className="modal">
					<div className="flex flex-column flex-gap-1">
						...
						{dataState.userPlaylists
							.filter(
								(playlist) => playlist.type === "user-created"
							)
							.map((playlist) => {
								return (
									<div
									...
									>
										<p className="form-check">
											<input
												onChange={(event) =>
													addOrRemovePlaylistVideo(
														event
													)
												}
												type="checkbox"
												checked={isVideoPresentInPlaylistVideos(
													video,
													playlist
												)}
												id={playlist._id}
